...
def build_global_masks(scene, gaussians, pipe, background, label_maps, args):
    """Complement Global Masking: render every train view with the warmed-up model,
    flag objects whose residual exceeds T_G, and track them across all frames via SAM2 video."""
    from global_masking import GlobalMaskTracker
    image_dir = os.path.join(args.source_path, args.images)
    tracker = GlobalMaskTracker(image_dir, args.sam2_ckpt)  # uses configs/sam2/sam2_hiera_l.yaml
    with torch.no_grad():
        for cam in scene.getTrainCameras():
            name = cam.image_name + ".jpg"
            lm = label_maps.get(name)
            if lm is None:
                continue
            pose = gaussians.get_RT(cam.uid)
            image = render(cam, gaussians, pipe, background, camera_pose=pose)["render"]
            gt_image = cam.original_image.cuda()
            Ll1 = l1_loss(image, gt_image)
            ssim_value = 1.0 - ssim(image, gt_image)
            combined_loss, _, _ = compute_combined_loss(Ll1, ssim_value)
            lm_t = torch.tensor(lm).cuda()
            heatmap_norm, _ = compute_instance_losses(
                combined_loss, lm_t, args.preset_instance_threshold, args.global_track_iter, cam.image_name, args)
            tracker.add_candidates(name, lm, heatmap_norm.detach().cpu().numpy(), args.lambda_global)
    tracker.run()
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    if args.use_voxel:
        gaussians.enable_voxel_guidance(N=args.voxel_N, tau=args.voxel_tau,
                                        gamma2=args.voxel_gamma2, gamma3=args.voxel_gamma3)
    if checkpoint:
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # ---- Complement Global Masking: build global distractor masks once, after warmup ----
        if args.use_global_mask and args.use_masks and iteration == args.global_track_iter:
            print(f"\n[ITER {iteration}] Building global masks (SAM2 video tracking)...")
            global_tracker = build_global_masks(scene, gaussians, pipe, background, label_maps, args)
        ...
        if args.use_masks and label_map_for_image is not None:
            ...
            # combine with global mask (1=keep static, 0=distractor) once available
            if global_tracker is not None and global_tracker.has_masks():
                gkeep = global_tracker.get_keep_mask(viewpoint_cam.image_name, heatmap_binary.shape[-2], heatmap_binary.shape[-1])
                if gkeep is not None:
                    heatmap_binary = heatmap_binary * gkeep
        ...
        with torch.no_grad():
            ...
            # Voxel-guided pruning: remove ghost-Gaussian voxels (low mean opacity). Run at a
            # controlled cadence and NOT right after an opacity reset (>600 iters since reset),
            # so freshly-reset opacities don't trigger a catastrophic prune.
            if args.use_voxel and iteration > opt.densify_from_iter and iteration % args.voxel_prune_interval == 0 \
                    and (iteration % opt.opacity_reset_interval) > 600:
                npr = gaussians.voxel_prune()
                if npr > 0:
                    print(f"[ITER {iteration}] voxel-pruned {npr} ghost Gaussians")
...
if __name__ == "__main__":
    ...
    # Complement Global Masking (re-implemented from paper Sec. 3.2)
    parser.add_argument("--use_global_mask", action="store_true", help="enable SAM2 video-tracking global distractor masking")
    parser.add_argument("--lambda_global", type=float, default=2.8, help="lambda_G: global threshold T_G = E[R] + lambda_G*Var[R]")
    parser.add_argument("--global_track_iter", type=int, default=1500, help="iteration at which to build global masks")
    # Voxel-guided optimization (re-implemented from paper Sec. 3.3)
    parser.add_argument("--use_voxel", action="store_true", help="enable voxel-guided gradient decay + ghost-voxel opacity pruning")
    parser.add_argument("--voxel_N", type=int, default=32, help="divide shortest scene-AABB edge into N voxels")
    parser.add_argument("--voxel_tau", type=float, default=3.5)
    parser.add_argument("--voxel_gamma2", type=int, default=1, help="min Gaussians per voxel before pruning")
    parser.add_argument("--voxel_gamma3", type=float, default=0.075, help="min mean voxel opacity before pruning")
    parser.add_argument("--voxel_prune_interval", type=int, default=500)
